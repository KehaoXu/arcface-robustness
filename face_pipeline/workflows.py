...
def run_condition_evaluation_pipeline(
    condition_name,
    condition_dir,
    baseline_dir,
    output_dir,
    ...
    baseline_dir = Path(baseline_dir)
    pair_dir = Path(pair_dir) if pair_dir is not None else baseline_dir
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    condition_artifacts = _extract_and_score_condition(
        embedder=face_embedder,
        sample_index_path=pair_dir / "sample_index.csv",
        condition_dir=condition_dir,
        output_dir=output_dir,
        condition_name=condition_name,
        pair_manifest_path=pair_dir / "pair_manifest.csv",
        eligible_only=True,
    )
    evaluate_condition_with_fixed_threshold(
